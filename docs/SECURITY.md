...
- Cross-user access checks are split by surface (see `supabase/tests/README.md` for how to run both and the findings list):
  - **Database RLS** is verified by SQL impersonation: `supabase/tests/rls_checks.sql` (transactional, self-rolling-back) exercises the full isolation matrix — read/update/delete/insert, own and cross-user, plus reference-table immutability — between two users (PRD §55, §59).
