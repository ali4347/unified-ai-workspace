...
- Ownership chains: where a user-owned row points at another user-owned row, the policy must verify the parent too, not just `user_id` — and on every command where the FK is mutable, not only INSERT. `messages` checks conversation ownership on INSERT and UPDATE (`20260825190000`); `attachments` checks it on INSERT (immutable thereafter); `provider_sessions` checks `connected_accounts` and `extension_devices` ownership on INSERT and UPDATE (`20260825180000`). Known remaining unverified references (integrity-only, awaiting product-owner decisions) are tracked in `supabase/tests/README.md` → Findings.
